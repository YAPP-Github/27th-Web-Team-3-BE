...
# 새 cron 추가 (매일 오전 9시 - 24시간에 1번)
# - .env 파일을 source하여 환경변수 로드
# - PATH 설정으로 jq, curl 등 명령어 사용 가능하게 함
# - 로그는 프로젝트 logs/ 디렉토리에 저장 (권한 문제 방지)
echo "0 9 * * * cd $PROJECT_DIR && export PATH=/usr/local/bin:/usr/bin:\$PATH && [ -f .env ] && set -a && . ./.env && set +a && ./scripts/log-watcher.sh >> $LOG_DIR/ai-monitor.log 2>&1" >> /tmp/crontab.tmp
