...
use super::dto::{
    ...
    JoinRetroRoomResponse, ReferenceItem, RetrospectDetailResponse, RetrospectListItem,
    RetrospectMemberItem, RetrospectQuestionItem, RetroRoomCreateRequest, RetroRoomCreateResponse,
    RetroRoomListItem, StorageQueryParams, StorageResponse, StorageRetrospectItem,
...
impl RetrospectService {
    ...
    pub async fn update_retro_room_order(
        ...
        for order_item in &req.retro_room_orders {
            if !member_room_ids.contains(&order_item.retro_room_id) {
                return Err(AppError::NoPermission("순서를 변경할 권한이 없습니다.".into()));
            }
        }
    ...
    pub async fn update_retro_room_name(
        ...
        Ok(UpdateRetroRoomNameResponse {
            ...
            updated_at: updated_room.updated_at.format("%Y-%m-%dT%H:%M:%S").to_string(),
        })
    }
    ...
    pub async fn delete_retro_room(
        ...
        if member_room.role != RoomRole::Owner {
            return Err(AppError::NoPermission("팀을 삭제할 권한이 없습니다.".into()));
        }
